/*! @file アプリケーションクラス CImageViewerの実装.
    ただし内部クラスの定義・実装は各ファイルにて行う。

    イベントハンドラonEvent()内で例外が発生した場合、
    その内容を表示したうえで再送出する。
    @ToDo
        イベントハンドラで例外が発生した場合、
        イベント自体をキャンセルし、例外再送（強制終了）を避ける
    @ToDo
        既知のバグ、特定のディスプレイから最大化した場合に、
        別のディスプレイに属する領域の描画が行われないことの対処。
    @ToDo
        ウィンドウを破棄する場合、win->destroy()を呼び出すだけでいいよう、
        Windowクラス側に、WM::DESTROYを処理するonEvent()のラッパーか
        なにかを噛ませる。あるいはdispatch関数内で処理させる。
        とにかく、create()とdestroy()を対照にすることと、
        イベントハンドラを書くときにポンプ機構を意識しなくていいように、
        PostQuitMessage()をWindowクラス内に隠蔽する。
*/
#ifndef STDAFX_H
#include <assert.h>
#include <algorithm>
